...
built, not while it is being handed over. If a local component turns out to
need design work, say so and name that skill; do not review it here with
half its vocabulary.
...
The rules for product copy belong to the `frontend-design` skill, which owns
them for the whole lifecycle — it is where they are applied while the screen is
being written, and restating them here would give a handoff two versions of
the same rule to drift apart. Load it and apply its copy section to the strings
this branch added.

Two checks are this gate's own, because they are about the handover rather
...
- **A control's label and its result agree.** "Publish" followed by "Saved" is
  two names for one action, and the second one is the one support gets asked
  about.
